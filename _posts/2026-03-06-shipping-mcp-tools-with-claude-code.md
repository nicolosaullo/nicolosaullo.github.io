...
The agent does the typing. The human does the thinking (which is great, keep doing that).

The bottleneck has moved from code generation to its verification. Did the code actually work? Did it break something else? Did it follow the project's standards?

The agent has no idea. It wrote the code and moved on.

The human is left with the worst piece of the job - spinning up a complex local environment and testing the changes manually, only to discover that they do not work.

What if this can be automated? What if we start treating agents as independent developers, and leverage our human abilities as AI pilots and technical leaders?

This would allow batches of user stories to be executed in parallel - but it requires a shift in how most teams currently work.
...
But they can also be run from the command line. And if they can be run from the command line, an agent can run them.
...
In this specific instance, I was building MCP tools for an AI chatbot. The flow looked like this:
...
Bruno CLI doesn't persist cookies or environment variables between separate `bru run` invocations. So the agent has to include the login step in every call and pass extracted values (like conversation IDs) explicitly via `--env-var`.

The skill encodes these quirks so the agent handles them automatically.

This same approach works with any API client that has a CLI. Postman (ugh) has `newman`. Insomnia has `inso`.
...
Second attempt passed. This is exactly the kind of failure that unit tests can't catch.
...
What used to take half a day of context switching and clicking around now takes one conversation.
